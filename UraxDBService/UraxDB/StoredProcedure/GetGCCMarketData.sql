CREATE PROCEDURE [dbo].[GetGCCMarketData]
(@PriceDate Varchar(max),@GCCPartnerCode varchar(max))
AS 
BEGIN
	--DECLARE @PriceDate varchar(max)='2017-12-01',@GCCPartnerCode varchar(10)='6AT2026'

	DECLARE @TEMP TABLE ( ID INT, GCCPartnerCode VARCHAR(MAX),PriceDate Date)
	INSERT INTO @TEMP
	SELECT T.Id, T.Value,T1.Value FROM
	...
	--SET @GCCPartnerCode=123;
	DECLARE @CountryCode varchar(100) , @TaxTerritory varchar(100);
	DECLARE @TEMP1 TABLE (  CountryCode VARCHAR(MAX),TaxTerritory varchar(100),PriceDate Date,GCCPartnerCode VARCHAR(MAX))
	INSERT INTO @TEMP1
	SELECT  cc.CountryCode,m.TaxTerritory, t.PriceDate,t.GCCPartnerCode FROM   PartnerCodeMapper pcm
	INNER JOIN dbo.Market m ON pcm.MarketId = m.MarketId
	INNER JOIN dbo.CountryCode cc ON m.CountryCodeId = cc.CountryCodeId
	INNER JOIN @TEMP t ON t.GCCPartnerCode = pcm.PartnerCode
	...
	,CAST(  CASE  WHEN  v.[Value] LIKE '%-%' THEN    substring(CAST(v.[Value] AS varchar(200)), CHARINDEX('-',v.[Value])+1, LEN(v.[Value])) ELSE UPPER(v.[Value])
	 ...
	 ,t1.PriceDate,t1.GCCPartnerCode
	...
	INNER JOIN @TEMP1 t1 ON t1.CountryCode =cc.CountryCode AND m.TaxTerritory = t1.TaxTerritory AND t1.PriceDate  BETWEEN tv.StartDate AND tv.EndDate
	...
	,UPPER(lug.LookUpGroupName) AS [LookUpGroupName]  ,lud.LookUpGroup,lud.GroupDetailsId
	...
	INNER JOIN @TEMP1 t1 ON t1.CountryCode =cc.CountryCode AND m.TaxTerritory = t1.TaxTerritory AND t1.PriceDate  BETWEEN tv.StartDate AND tv.EndDate
	WHERE   f.FormulaDefination IS  NULL
	...
	LEFT JOIN dbo.Formula f ON v.VariableId = f.VariableId
	INNER JOIN @TEMP1 t1 ON t1.CountryCode =cc.CountryCode AND m.TaxTerritory = t1.TaxTerritory AND t1.PriceDate  BETWEEN tv.StartDate AND tv.EndDate 
	WHERE	 f.FormulaDefination IS NOT NULL
	)
	GROUP BY tm.TaxName,tm.TaxMasterId,tv.TaxVersionId,tv.StartDate, tv.EndDate,pd4.[Value]  ,t.TaxFlowId,t1.PriceDate,t1.GCCPartnerCode
